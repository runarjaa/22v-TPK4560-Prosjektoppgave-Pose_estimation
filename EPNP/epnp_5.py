...
class EPnP:
    ...
    def compute_points(self):
        ...
        for i, p in enumerate(self.pix):
            if i % a == 0:
                p[0] += rand.randint(smin, smax) 
                p[1] += rand.randint(smin, smax) 
 ...
 # -----------------------------------------------------------------------------
  # -----------------------------------------------------------------------------
   # -----------------------------------------------------------------------------
    ...
    # Did not understand this, so this is on loan from EPnP python
    ...
    def getRotT(self, wpts, cpts):
        ...
        test = np.diag([1,1,np.linalg.det( Vt.T @ U.T)])
        R = Vt.T @ test @ U.T
...
# Error
    ...
    def plot_results_o3d(self):
        ...
        # Pixel with outliers
        pcd_outl = o3d.geometry.PointCloud()
        pcd_outl.points = o3d.utility.Vector3dVector(self.pix)
        pcd_outl.paint_uniform_color(color2)
        ...
        test = np.asarray(pcd_epnp.points)

        for i, v in enumerate(test):
            if np.linalg.norm(test[i]) > 5000:
                pcd_epnp.points[i] = zero
        
        pcd_epnp.points = o3d.utility.Vector3dVector(test)
